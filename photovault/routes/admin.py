...
def statistics():
    ...
    # Photo statistics - manual calculation
    all_photos = Photo.query.all()
    total_photos = len(all_photos)
    edited_photos = sum(1 for photo in all_photos if photo.edited_filename is not None)
    original_only_photos = total_photos - edited_photos
    
    # Storage statistics
    total_size = sum(photo.file_size or 0 for photo in all_photos)
    avg_file_size = total_size / total_photos if total_photos > 0 else 0
    
    # Recent activity (last 30 days)
    thirty_days_ago = datetime.utcnow() - timedelta(days=30)
    recent_uploads = sum(1 for photo in all_photos if photo.uploaded_at and photo.uploaded_at >= thirty_days_ago)
    recent_users = sum(1 for user in all_users if user.created_at and user.created_at >= thirty_days_ago)
    
    # Most active users - simple calculation
    user_photo_counts = []
    for user in all_users:
        photo_count = sum(1 for photo in all_photos if photo.user_id == user.id)
        if photo_count > 0:
            user_photo_counts.append((user.username, photo_count))
    
    # Sort and take top 10
    most_active_users = sorted(user_photo_counts, key=lambda x: x[1], reverse=True)[:10]
    ...
    statistics = {
        ...
        'photos': {
            ...
            'original_only': original_only_photos,
        ...
        'storage': {
            ...
            'avg_file_size_kb': round(avg_file_size / 1024, 2) if avg_file_size else 0
        },
