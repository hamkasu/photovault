...
@photo_bp.route('/photo/<int:photo_id>/tag_person', methods=['POST'])
@login_required
def tag_person_in_photo(photo_id):
    """Tag a person in a photo with optional position data"""
    photo = Photo.query.filter_by(id=photo_id, user_id=current_user.id).first_or_404()
    
    data = request.get_json()
    person_id = data.get('person_id')
    
    # Optional position data for face detection
    bbox_x = data.get('bbox_x', 0)
    bbox_y = data.get('bbox_y', 0) 
    bbox_width = data.get('bbox_width', 0)
    bbox_height = data.get('bbox_height', 0)
    confidence = data.get('confidence', 1.0)
    manually_tagged = data.get('manually_tagged', True)
    ...
    if existing:
        return jsonify({'error': f'{person.name} is already tagged in this photo'}), 400
    
    # Create photo-person relationship
    photo_person = PhotoPerson(
        ...
        verified=manually_tagged  # Manual tags are automatically verified
    )
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'Tagged {person.name} in photo',
            'person': {
                'id': person.id,
                'name': person.name,
        ...
        db.session.rollback()
        logger.error(f'Error tagging person: {str(e)}')
        return jsonify({'error': 'Failed to tag person'}), 500

@photo_bp.route('/photo/<int:photo_id>/untag_person/<int:person_id>', methods=['DELETE'])
@login_required
def untag_person_from_photo(photo_id, person_id):
    """Remove a person tag from a photo"""
    photo = Photo.query.filter_by(id=photo_id, user_id=current_user.id).first_or_404()
    
    # Find the photo-person relationship
    photo_person = PhotoPerson.query.filter_by(photo_id=photo.id, person_id=person_id).first()
    if not photo_person:
        return jsonify({'error': 'Person not tagged in this photo'}), 404
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'Removed {person.name} from photo'
        })
    except Exception as e:
        db.session.rollback()
        logger.error(f'Error removing person tag: {str(e)}')
        return jsonify({'error': 'Failed to remove person tag'}), 500

@photo_bp.route('/people/search')
@login_required
def search_people():
    """Search people by name for tagging autocomplete"""
